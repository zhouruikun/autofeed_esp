...
void led_on(uint8_t led)
{
			...
			GPIO_OUTPUT_SET(GPIO_ID_PIN(led_buff[led].pin), led_buff[led].active_level);  
...
void led_off(uint8_t led)
{
		...
		{
			 GPIO_OUTPUT_SET(GPIO_ID_PIN(led_buff[led].pin), 1);  
 
		}
		else
		{
			 GPIO_OUTPUT_SET(GPIO_ID_PIN(led_buff[led].pin), 0);  
